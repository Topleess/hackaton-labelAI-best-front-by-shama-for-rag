    ...
    if (typeof messageText === "string" && messageText.trim()) {
      try {
        ...
        if (!Array.isArray(messages)) {
          console.error(
            "messages is not an array, initializing as empty array."
          );
        ...
        setMessages((prevMessages) => {
          if (!Array.isArray(prevMessages)) {
            console.error(
              "prevMessages is not an array, initializing as empty array."
            );
        ...
        const response = await fetch(
          `${process.env.REACT_APP_HTTP_URL}/api/chat`,
          {
            method: "POST",
            headers: {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          {
            text_data: { content: "", role: "assistant" },
            rag_data: data.data,
          },
        ]);
      } catch (error) {
        ...
        if (botReply.llm_message) {
          if (botReply.llm_message === "<|endoftext|>") {
            ...
            setMessages((prevMessages) => {
              ...
              const lastBotMessageIndex = updatedMessages.findLastIndex(
                (msg) => msg.text_data.role === "assistant"
              );
              ...
              if (lastBotMessageIndex !== -1) {
                updatedMessages[
                  lastBotMessageIndex
                ].text_data.content += `${botReply.llm_message}`;
              } else {
