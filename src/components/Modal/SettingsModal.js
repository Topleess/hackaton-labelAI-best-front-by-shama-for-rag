      ...
      <ModalContainer>
        ...
        <CardRow>
          {['model1', 'model2', 'model3'].map((item) => (
            ...
            >
              Модель {item.slice(-1)}
              <CardDescription>Описание модели {item.slice(-1)}</CardDescription>
            </Card>
          ))}
        </CardRow>
        ...
        <CardRow>
          {['method1', 'method2', 'method3'].map((item) => (
            <Card
              key={item}
              className={searchMethod === item ? 'selected' : ''}
              onClick={() => setSearchMethod(item)}
            >
              Метод {item.slice(-1)}
              <CardDescription>Описание метода {item.slice(-1)}</CardDescription>
            </Card>
          ))}
        </CardRow>
        ...
        <CardRow>
          {['withInterpretation', 'withoutInterpretation'].map((item) => (
            <Card
              key={item}
              className={responseType === item ? 'selected' : ''}
              onClick={() => setResponseType(item)}
            >
              {item === 'withInterpretation' ? 'С интерпретацией' : 'Без интерпретации'}
              <CardDescription>
                Пример {item === 'withInterpretation' ? 'с интерпретацией' : 'без интерпретации'}
              </CardDescription>
              <Image
                src={`/${item === 'withInterpretation' ? 'interpretation-example.png' : 'no-interpretation-example.png'}`}
                alt={`Пример ${item === 'withInterpretation' ? 'с интерпретацией' : 'без интерпретации'}`}
              />
            </Card>
          ))}
        </CardRow>
        ...
        {/* Плашка с точностью */}
        <AccuracyInfo>Точность выбранной конфигурации: {calculateAccuracy()}</AccuracyInfo>
